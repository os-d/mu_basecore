  ...
  )
{
  ...
  BinRange.BaseAddress                = MAX_UINTN;
  BinRange.Length                     = 0;
  BinEnd                              = 0;

  gBS->AllocatePages(AllocateAnyPages, EfiReservedMemoryType, 0x40,
  ...
  for (Index = 0; PreviousMemoryTypeInformation[Index].Type != EfiMaxMemoryType; Index++) {
    ...
    DEBUG ((DEBUG_ERROR, "OSDDEBUG506 Type %u: InBin 0x%llx OutOfBin 0x%llx\n",
      CurrentMemoryTypeInformation->Statistics[Index1].Type,
         ...
         CurrentMemoryTypeInformation->Statistics[Index1].CurrentNumberOfPagesOutOfBin < Previous))
    {
      ...
      Current = Previous + (UINT32)CurrentMemoryTypeInformation->Statistics[Index1].CurrentNumberOfPagesOutOfBin;
    } else {
      ...
      Current = (UINT32)(CurrentMemoryTypeInformation->Statistics[Index1].CurrentNumberOfPagesInBin +
                         CurrentMemoryTypeInformation->Statistics[Index1].CurrentNumberOfPagesOutOfBin);
    }
  ...
  if (MemoryTypeInformationModified || !MemoryTypeInformationVariableExists) {
    ...
    BinRange.Length = ((BinEnd + 1) & ~EFI_PAGE_MASK) - BinRange.BaseAddress;
    DEBUG ((DEBUG_INFO, "OSDDEBUG505 Memory Type Information Bins Range: 0x%llx - 0x%llx\n", BinRange.BaseAddress, BinRange.Length + BinRange.BaseAddress - 1));
